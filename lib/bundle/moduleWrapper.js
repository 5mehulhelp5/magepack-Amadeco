...
/*
 * LEGACY wrapNonAmd — kept for reference, superseded by the implementation below.
 * The original version did not include the jQuery concurrency race fix.
 *
 ...
 *
 * Reads shim configuration from RequireJS context to preserve declared
 * dependencies and exports, matching the behaviour of `r.js` optimizer shims.
 *
 * Includes a concurrency race fix: if another script hijacked `window.jQuery`
 * during async bundle loading, the AMD-registered instance is restored before
 * the wrapped plugin executes, preventing `$.fn` extension failures.
 *
 * @param {string} moduleName - RequireJS module ID.
 * @param {string} content - Raw source of the legacy (non-AMD) script.
 * @returns {string} AMD `define()` block wrapping the original content.
 */
export const wrapNonAmd = (moduleName, content) => {
    return `define('${moduleName}', (require.s.contexts._.config.shim['${moduleName}'] && require.s.contexts._.config.shim['${moduleName}'].deps || []), function() {

    // --- CONCURRENCY RACE FIX ---
    // Legacy plugins strictly rely on the global 'window.jQuery' object.
    // If an external script hijacked the global scope during async bundle loading, 
    // we force restore the canonical AMD jQuery instance right before execution.
    if (typeof require !== 'undefined' && require.defined('jquery')) {
        var reqJquery = require('jquery');
        if (window.jQuery !== reqJquery) {
            window.jQuery = window.$ = reqJquery;
        }
    }
    // ----------------------------

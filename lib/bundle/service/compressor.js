...
export const compressFile = async (filePath, options = {}) => {
    ...
    // allSettled: one format failing (e.g., disk full on .gz) does not abort the others.
    const results = await Promise.allSettled([gzipJob, brotliJob, zstdJob()]);
    for (const result of results) {
        if (result.status === 'rejected') {
            consola.error(`❌ Compression task failed for ${filePath}: ${result.reason?.message ?? result.reason}`);
        }
    }
};

 ...
 * @type {boolean|null}
 */
let isZstdInstalled = null;

/**
 * Checks if the 'zstd' CLI tool is installed on the host OS.
 * @returns {Promise<boolean>}
 */
const checkZstdAvailability = async () => {
    if (isZstdInstalled !== null) return isZstdInstalled;
    try {
        await execFileAsync('zstd', ['--version']);
        isZstdInstalled = true;
    } catch (error) {
        isZstdInstalled = false;
    }
    return isZstdInstalled;
};
    ...
    const zstdJob = async () => {
        ...
        if (!hasZstd) {
            ...
            if (isZstdInstalled === false) {
                ...
                isZstdInstalled = 'warned'; // Prevent duplicate warnings
            }

...
export const composeMixinTarget = (targetId, targetContent, mixinSources) => {
    ...
    // Use JSON.stringify for safe embedding in generated JS strings.
    // This handles all edge cases: backslashes, quotes, control characters,
    // and Unicode — unlike manual .replace() chains which only cover simple cases.
    const quotedTargetId = JSON.stringify(targetId);
    ...
    // JSON.stringify produces correctly quoted+escaped string literals for all IDs.
    const compositeDeps = [
        JSON.stringify(renamedTargetId),
        ...mixinSources.map((s) => JSON.stringify(s.mixinId)),
    ];
    ...
    const orchestratorLines = [
        ...
        `define(${quotedTargetId}, [${compositeDeps.join(', ')}], function(${compositeParams.join(', ')}) {`,
        `    "use strict";`,
        `    return ${chainExpr};`,
        `});`,
    ];
    ...
    const parts = [
        ...
        ensureTrailingSemicolon(orchestratorLines.join('\n')),
    ];

 ...
 * 1. The modified requirejs-config.js AND requirejs-config.min.js
...
export const updateSriHashes = async (locales, bundles) => {
    try {
        ...
        for (const locale of locales) {
            ...
            // CORRECTION 1 : Normalisation du chemin pour la clé JSON (toujours des slashs /)
            const localeKey = localePathRelative.split(path.sep).join('/');
            
            const localePathAbsolute = path.join(rootPath, PATHS.STATIC_FRONTEND, localePathRelative);

            // CORRECTION 2 : Mise à jour des hashes pour .js ET .min.js
            const configFiles = [FILES.REQUIREJS_CONFIG, FILES.REQUIREJS_CONFIG_MIN];

            for (const configFile of configFiles) {
                const configKey = `${localeKey}/${configFile}`;
                
                try {
                    const configBuffer = await fs.readFile(path.join(localePathAbsolute, configFile));
                    sriData[configKey] = generateSriHash(configBuffer);
                    updateCount++;
                } catch (e) { 
                    // Le fichier peut ne pas exister (ex: .min.js en mode dev), on ignore.
                }
            }
            ...
            for (const bundle of bundles) {
                ...
                for (const ext of extensions) {
                    ...
                    // Utilisation de localeKey ici aussi pour garantir les slashs corrects

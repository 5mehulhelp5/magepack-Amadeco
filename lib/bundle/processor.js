...
/**
 * Sensitive module patterns that force "safe" Terser strategy regardless of --minify-strategy.
 *
 * Intentionally empty by default. Populate with RegExp patterns for modules that break
 * under aggressive minification. Common candidates:
 *
 *   - Payment SDKs using eval() or new Function() (Braintree, Stripe, Adyen, Klarna)
 *   - Libraries detecting their own function names via Function.prototype.name (jQuery plugins)
 *   - KnockoutJS computed/observable introspection relying on Function.name
 *   - Modules using arguments.callee (non-strict-mode semantics)
 *
 * Example entries:
 *   /^jquery\//,
 *   /^knockoutjs\//,
...
export const processBundle = async (bundle, localePath, outputDir, options, isMinifyOn, mixinMap, allMixinModuleIds) => {
    ...
    // Batch size limits concurrent open file handles to prevent OS-level EMFILE errors.
    // Configurable via --batch-size CLI option (default: 50, clamped to [1, 500]).
    const parsedBatchSize = parseInt(options.batchSize, 10);
    const BATCH_SIZE = (Number.isFinite(parsedBatchSize) && parsedBatchSize >= 1 && parsedBatchSize <= 500)
        ? parsedBatchSize
        : (() => {
            if (options.batchSize !== undefined && options.batchSize !== '50') {
                consola.warn(`⚠️  Invalid --batch-size "${options.batchSize}" (must be 1–500). Using default: 50.`);
            }
            return 50;
        })();
        ...
        await Promise.all(chunk.map(async (moduleName) => {
            try {
                ...
                if (!absPath) {
                    ...
                    consola.warn(`⚠️  [${bundle.name}] Module missing: "${moduleName}" → ${mappedPath}`);
                    return;
                }

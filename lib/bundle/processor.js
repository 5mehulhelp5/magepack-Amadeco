...
export const processBundle = async (bundle, localePath, options, isMinifyOn) => {
    ...
    // The folder where bundles are stored: .../fr_FR/magepack
    ...
    for (const moduleName of moduleNames) {
        try {
            ...
            // Generate content with AMD wrapper
            const wrappedContent = moduleWrapper(moduleName, content, absPath);

            // KEY CHANGE FOR SOURCEMAPS:
            // Calculate relative path from the 'magepack' folder to the source file.
            // Ex: ../Magento_Catalog/js/product.js
            // This allows the browser to find the file via HTTP request relative to the bundle location.
            const relativeSourcePath = path.relative(destDir, absPath);

            // Use relative path as key so the .map file points to it
    ...
    if (shouldMinifyContent || shouldGenerateSourceMap) {
        try {
            ...
            // CRITICAL: Force disable content embedding.
            // This ensures the .map file contains only paths (filenames), not the code itself.
            if (shouldGenerateSourceMap) {
                if (!terserOptions.sourceMap) terserOptions.sourceMap = {};
                terserOptions.sourceMap.includeSources = false;
            }

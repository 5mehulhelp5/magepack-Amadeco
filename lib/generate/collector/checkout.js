...
const checkout = async (browserContext, config) => {
    ...
    try {
        ...
        // Ensure the button is actually initialized and clickable by Magento
        await page.waitForSelector(selectors.addToCartButton, { visible: true });

        // We attempt to click the button and wait for the Magento success message or redirect
        try {
            await Promise.all([
                // Wait for EITHER a navigation (redirect mode) OR the Magento success message (AJAX mode)
                Promise.race([
                    page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 15000 }).catch(() => {}),
                    // Wait for the standard Magento success message to appear in the DOM
                    page.waitForSelector('.message-success', { timeout: 15000 }).catch(() => {
                        logger.warn('AJAX Success message did not appear. Cart might be empty.');
                    })
                ]),
                // Click the button
            ...
            // Allow a tiny buffer for Magento's mini-cart to update its state internally
        ...
        logger.debug(`📍 Current URL after Cart load: ${page.url()}`);
        
        const cartModules = await collectModules(page);

        logger.debug(`📦 Collected ${Object.keys(cartModules).length} modules from the Cart page.`);
        ...
        logger.debug(`📍 Current URL after Checkout load: ${page.url()}`);
        if (page.url().includes('cart')) {
            logger.warn('⚠️ WARNING: Magento redirected the checkout back to the cart! The cart is likely empty.');
        }
        
        const checkoutModules = await collectModules(page);

        logger.debug(`📦 Collected ${Object.keys(checkoutModules).length} modules from the Checkout page.`);

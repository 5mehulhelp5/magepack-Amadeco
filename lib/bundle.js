 ...
 *
 * @changelog
 *   - v3.1.0: Added mixin-aware module exclusion via `mixinResolver` service.
 *     Modules that are mixin targets or mixin factories are now automatically
 *     removed from bundle configurations before processing, restoring proper
 *     RequireJS `mixins!` plugin interception for individually-loaded modules.
...
import { resolveMixinExclusions } from './bundle/service/mixinResolver.js';
 ...
 * the new mixin exclusion logic mutate `bundle.modules` in place.
 *
...
/**
 * Removes mixin-affected modules from all bundles in the configuration.
 *
 * Two categories of modules are removed:
 *   1. **Mixin targets**: Modules that have active `config.mixins` entries.
 *      These must be loaded individually so `require.load()` fires and the
 *      `mixins!` plugin can intercept and apply the mixin chain.
 *   2. **Mixin modules**: The factory functions that wrap mixin targets.
 *      These are never `require()`d by application code — only by the `mixins!`
 *      plugin. Bundling them is dead code.
 *
 * @param {Array<Object>} config - The bundle configuration array (will be mutated).
 * @param {{ targets: Set<string>, mixinModules: Set<string> }} exclusions - The sets
 *   of module IDs to exclude, as returned by `resolveMixinExclusions()`.
 * @returns {Array<Object>} The same config array with mixin modules removed.
 */
const applyMixinExclusions = (config, exclusions) => {
    const { targets, mixinModules } = exclusions;

    if (targets.size === 0 && mixinModules.size === 0) {
        return config;
    }
    ...
    config.forEach((bundle) => {
        ...
        Object.keys(bundle.modules).forEach((moduleName) => {
            if (targets.has(moduleName) || mixinModules.has(moduleName)) {
                delete bundle.modules[moduleName];
            }
        });

        const removed = originalCount - Object.keys(bundle.modules).length;

        if (removed > 0) {
            totalRemoved += removed;
            consola.debug(
                `   🔗 [${bundle.name}] Excluded ${removed} mixin-affected module(s).`
            );
        }
    });
    ...
    if (totalRemoved > 0) {
        consola.info(
            `   🔗 Total: ${totalRemoved} module(s) excluded for mixin compatibility.`
        );
    }
...
async function processLocale(locale, sharedConfig, options) {
    ...
    try {
        ...
        //     Both ghost module pruning (processor.js) and mixin exclusion
        ...
        // 1.6 MIXIN EXCLUSION: Detect and remove mixin-affected modules.
        //     This restores RequireJS mixins! plugin interception for these modules
        //     by forcing them to load individually via require.load().
        const mixinExclusions = await resolveMixinExclusions(localePath, isMinifyOn);
        applyMixinExclusions(localeConfig, mixinExclusions);

        // 2. BUILD: Generate bundles into the temporary directory
        await Promise.all(
            localeConfig.map(bundle => processBundle(bundle, localePath, buildDir, options, isMinifyOn))
        );

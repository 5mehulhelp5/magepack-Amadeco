...
const TERSER_STRATEGIES = {
    ...
    aggressive: {
        // ES2019 is a safe sweet spot: modern syntax but stable scoping
        ecma: 2019, 
        compress: {
            passes: 2,           // Reduced from 3 to 2 to avoid over-inlining
            drop_console: true,  // Still remove logs for performance
            drop_debugger: true,
            
            // --- SECURITY FIXES FOR MAGENTO ---
            unused: false,       // CRITICAL: Prevent dropping function args that map to RequireJS deps
            side_effects: false, // CRITICAL: Prevent dropping "pure" calls that actually register plugins
            sequences: false,    // CRITICAL: Keep execution order strict
            collapse_vars: false,// CRITICAL: Avoid complex variable merging bugs in legacy scripts
            
            // --- SAFE OPTIMIZATIONS ---
            pure_getters: false, // Too risky for legacy jQuery plugins
            unsafe: false,
            unsafe_proto: false,
            dead_code: true,     // Safe to remove unreachable code
            conditionals: true,
            booleans: true,
            if_return: true,
            join_vars: true,

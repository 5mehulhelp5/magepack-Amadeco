...
/**
 * Compresses a file using Extreme Gzip and Tuned Brotli settings.
 * * @param {string} filePath - Path to the original file.
 * @returns {Promise<void>}
 */
const compressFile = async (filePath) => {
    try {
        // We read file stats/content to provide hints to the compressor (better memory alloc)
        ...
        await pipeline(
            ...
            createGzip({ 
                ...
                memLevel: 9, // Use max memory for compression buffer
                windowBits: 15 // Max window size for Gzip
            }),
            createWriteStream(`${filePath}.gz`)
        );

        // --- BROTLI (Max Quality + Max Window) ---
        await pipeline(
            createReadStream(filePath),
            createBrotliCompress({ 
                params: { 
                    [zlibConstants.BROTLI_PARAM_MODE]: zlibConstants.BROTLI_MODE_TEXT,
                    [zlibConstants.BROTLI_PARAM_QUALITY]: zlibConstants.BROTLI_MAX_QUALITY, // Level 11
                    [zlibConstants.BROTLI_PARAM_LGWIN]: 24, // Max Window Size (up from default 22). Critical for big bundles.
                    [zlibConstants.BROTLI_PARAM_SIZE_HINT]: fileStat.size // Helps optimizer
                } 
            }),

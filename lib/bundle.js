...
const processBundle = async (bundle, localePath, options) => {
    ...
    // CORRECTION : Instanciation correcte via la Factory importée de moduleMapResolver
    // On crée une fonction de résolution spécifique pour cette locale
    const resolvePath = createPathResolver(localePath, options.minify);
    ...
    for (const moduleName of moduleNames) {
        ...
        try {
            // CORRECTION : Utilisation de l'instance du résolveur
            // resolvePath est maintenant une fonction synchrone renvoyée par moduleMapResolver
            const absolutePath = resolvePath(modulePath);
            
            // On vérifie l'existence du fichier avant lecture
            ...
            // On log en debug/warn plutôt qu'en erreur bloquante pour ne pas arrêter tout le process si un fichier manque

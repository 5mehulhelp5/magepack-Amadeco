...
const buildRequireConfigContent = (config, isMinifyOn) => {
    ...
    config.forEach((bundle) => {
        if (!bundle || !bundle.name || !Array.isArray(bundle.files)) {
        ...
        // Map bundle name to its included modules
        bundles[bundleName] = bundle.files.map((f) => f.replace(/\.js$/, ''));
    });
...
async function mergeConfigs(localePath, isMinifyOn) {
    ...
    try {
        ...
        // We check for a known string unique to our bundle config
        ...
        // Warning instead of Error: The build should continue, but the site might be slow/broken
        if (e.code === 'ENOENT') {
...
export default async (configPath, globPattern, sourcemap, minify, minifyStrategy, theme) => {
    ...
    for (const locale of locales) {
        ...
        try {
            ...
            // If CLI flag 'minify' is true, we force it. Otherwise, we detect from file system.
            ...
            // Parallel execution for all bundles in the config
            ...
            // This injects the bundle definition into the main requirejs-config.js/.min.js
            await mergeConfigs(localePath, isMinifyOn);

        } catch (e) {
            consola.error(`Failed ${label}:`, e);
            // We do not exit process here to allow other locales to finish
        }
    }

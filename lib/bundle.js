...
const processBundle = async (bundle, localePath, options) => {
    ...
    // 2. Minify Content (Aggressive Strategy)
    let finalContent = bundleContent;
    // We enforce aggressive minification for production performance
        ...
        try {
            const result = await minify(bundleContent, {
                ...
                compress: {
                    ...
                    // This is safe because loggers are usually only for debugging
 ...
 *
 * @param {Array<Object>} config - The bundling configuration.
...
export default async (config, options) => {
    ...
    for (const localePath of localesPaths) {
        ...
        try {
            // 1. Process all bundles in parallel for this locale
            // This includes 'vendor', 'common', and all page-specific bundles
            await Promise.all(
                config.map((bundle) => processBundle(bundle, localePath, options))
            );

            // 2. Generate the wiring configuration (requirejs-config-common.js)
            ...
            // We continue to the next locale instead of exiting
        }
    }

...
const generateRequireConfig = async (bundles, localePath, isMinifyOn) => {
    const suffix = isMinifyOn ? '.min' : ''; // Définition du suffixe
    ...
    for (const bundle of bundles) {
        bundlesConfig[`magepack/bundle-${bundle.name}${suffix}`] = Object.keys(bundle.modules || {});
    }

    if (bundles.some((b) => b.name === 'vendor')) deps.push(`magepack/bundle-vendor${suffix}`);
    if (bundles.some((b) => b.name === 'common')) deps.push(`magepack/bundle-common${suffix}`);

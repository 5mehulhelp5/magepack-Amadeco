...
const processBundle = async (bundle, localePath, options, isMinifyOn) => {
    ...
    for (const moduleName of moduleNames) {
        ...
        try {
            // FIX 1: Correction du nom de variable (rawPath -> rawModulePath)
            const mappedPath = resolveMap(rawModulePath);
            
            const absPath = await resolveFile(localePath, moduleName, mappedPath, isMinifyOn);
            
            // FIX 2: Suppression de la double déclaration et usage de la bonne variable (absPath)
            ...
            // FIX 3: Utilisation de absPath (et non absolutePath)

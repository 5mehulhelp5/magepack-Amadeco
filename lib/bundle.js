...
const processBundle = async (bundle, localePath, options) => {
    ...
    // --- CORRECTION : Instanciation correcte du PathResolver (Factory) ---
    // On initialise le résolveur une seule fois pour ce contexte (Locale + Minification)
    const resolvePath = pathResolver(localePath, options.minify);
    ...
    for (const moduleName of moduleNames) {
        ...
        try {
            ...
            // pathResolver retourne une fonction synchrone, pas besoin d'await ici
            const absolutePath = resolvePath(modulePath);
            
            // On vérifie l'accessibilité avant de lire (évite les erreurs silencieuses si chemin incorrect)
